...
class ubike_db():
    ...
    def connect2db(self):
        if POSTGRESQL_DB:
            self.conn = psycopg2.connect(database="ubike_db", user="postgres", password="0719", host="localhost", port="5432")
        else:
...
class weather_db():
    ...
    def connect2db(self):
        if POSTGRESQL_DB:
            self.conn = psycopg2.connect(database="weather_db", user="postgres", password="0719", host="localhost", port="5432")
        else:
